...
<%@ include file="/emadmin/shared/checkPermission1.jsp"%>
		<%
				...
				String old_group_id=(String)request.getParameter("old_group_id");
				String title=(String)request.getParameter("title");
				String message="";
				if (message.equals("")) {
					try {						
						OaService.update(id,userid,content, request.getRemoteAddr(),group_id,title);
						response.sendRedirect("notebook.jsp?group_id="+old_group_id);
					} catch (Exception e) {
      ...
      <tr>
        <td valign="top"><font class="textNormal">
            <table class="newsListbody" cellSpacing="10" cellPadding="0" width="100%" border="0">
              <tr>
                <td height="50" align="left" valign="bottom"><font class="textNormal">  &nbsp;&nbsp;<a href="notebook.jsp" class="link_rc">记事本</a></font></td>
              </tr>
              <tr>
...
<% if (message.equals("") ){ %>
  <tr> 
    <td  align="center" valign="top">您的信息已经成功保存!<br>
	  <input type="button" name="Submit" value="确定" class="anniu" onClick="javascript:location.href='notebook.jsp?group_id=<%=old_group_id%>'">
	</td>
  </tr>
  <%} else {%>
  <tr> 
...
function docheck()
{ 
	if (form1.content.value.length>254) {
		alert('记事本每条内容不能超过128个汉字，请精简！');
		form1.content.focus();
		return false;
	}
